...
def show_angle(speed_limit):
    if speed_limit !=0:
        show_direction = cv2.imread("/home/lionel/Desktop/TDI_speed/speedDetection/PromptAngleInfo.JPG")
...
#make line
vs = VideoStream()
vs.stream = cv2.VideoCapture("night.mp4")
frame = vs.read()
...
while True:
    ...
    ## to count the number of cars that crossed the cross_frame1
    cross_frame_count = 0
    ...
    if flag is False and max_cy_ > midtrack and max_cy_ < (midtrack+100):
        ...
        # while frame_crossed1:
        #     cross_frame_count += 1
        #     break
        #     # print('cross frame count : ', cross_frame_count)
        speed_insta = (distance/frame_crossed1)*fps*3.6
        
        speed = speed_insta*Angle
        print("SPEED",speed)
        print("frame_crossed1",frame_crossed1)
        print('cross_frame1_count: ', cross_frame_count)
